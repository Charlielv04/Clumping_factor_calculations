...
def _install_fake_simloader(monkeypatch):
    module = types.ModuleType("simloader")
    gadget = types.SimpleNamespace()

    def compute_particles_temperature(u, UnitVelocity_in_cm_per_s=1e5, MeanMolecularWeight=1, gamma=5 / 3):
        return np.asarray(u, dtype=np.float64) * float(MeanMolecularWeight) * (float(UnitVelocity_in_cm_per_s) / 1e5) ** 2

    gadget.computeParticlesTemperature = compute_particles_temperature
    module.gadget = gadget
    monkeypatch.setitem(sys.modules, "simloader", module)
    return compute_particles_temperature


def test_compute_snapshot_temperature_uses_simloader_and_volume_weighting(tmp_path, monkeypatch):
...
def test_temperature_cache_written_beside_snapshot(tmp_path, monkeypatch):
    _install_fake_simloader(monkeypatch)
    base = _write_snapshot(tmp_path / "snapshot")
...
def test_temperature_weighting_option_changes_summary(tmp_path, monkeypatch):
    _install_fake_simloader(monkeypatch)
    base = _write_snapshot(tmp_path / "snapshot")
    path = base / "snapdir_080" / "snap_080.0.hdf5"
...
def test_equations_can_use_cell_local_recombination_temperature(tmp_path, monkeypatch):
    _install_fake_simloader(monkeypatch)
    base = _write_snapshot(tmp_path / "snapshot")
    common = dict(
        base_path=base,
...
def test_equation_cli_computes_missing_temperature_table(tmp_path, monkeypatch):
    _install_fake_simloader(monkeypatch)
    base = _write_snapshot(tmp_path / "snapshot")
...
def test_temperature_cli_writes_json_output(tmp_path, monkeypatch):
    _install_fake_simloader(monkeypatch)
    base = _write_snapshot(tmp_path / "snapshot")
