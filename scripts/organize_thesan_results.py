#!/usr/bin/env python3
"""Audit and optionally organize Thesan result files into the canonical layout."""
...
THESAN_RE = re.compile(r"Thesan-[12]")
MANIFEST_COLUMNS = [
    "file_type",
    "source_path",
    "canonical_path",
...
def thesan_simulation(*candidates: Any) -> str:
    for candidate in candidates:
        if not candidate:
            continue
        match = THESAN_RE.search(str(candidate))
        if match:
...
def parse_filename(path: Path) -> dict[str, Any]:
    if match := OLD_RESULT_RE.match(path.name):
...
def source_campaign(path: Path, document: dict[str, Any]) -> str:
    ...
    for part in reversed(path.parts):
        if part.startswith("Thesan-") and part != "Thesan-1" and part != "Thesan-2":
            return part
...
def parse_result(path: Path, results_root: Path) -> dict[str, Any]:
    ...
    row = {
        ...
        "threads": str(filename.get("threads") or parameters.get("threads") or first_nested(document, "requested_threads", "threads", default="")),
...
def canonical_path(results_root: Path, row: dict[str, Any], run_index: int) -> str:
    return str(
        ...
        / f"snapshot{int(row['snapshot']):03d}_grid{int(row['grid'])}"
...
def thesan_json_paths(results_root: Path, include_canonical: bool = False) -> list[Path]:
    paths: list[Path] = []
    for directory in results_root.glob("Thesan-*"):
        if directory.is_dir():
            paths.extend(directory.rglob("*.json"))
    canonical = results_root / "thesan"
    if include_canonical and canonical.exists():
        paths.extend(canonical.rglob("*.json"))
    return sorted(set(paths))


def thesan_plot_paths(results_root: Path) -> list[Path]:
...
def is_canonical_analysis_path(path: Path) -> bool:
    ...
    for index, part in enumerate(parts):
        if part == "analysis" and index + 2 < len(parts):
            if parts[index + 1] in {"performance", "clumping", "cell-count", "misc"} and parts[index + 2] == "thesan":
                return True
    return False
...
def infer_plot_destination(results_root: Path, plot_path: Path) -> Path:
    ...
    simulation = thesan_simulation(plot_path)
    ...
    return results_root / "analysis" / plot_type / "thesan" / simulation / snapshot / particle / backend / plot_path.name
...
def build_reports(
    ...
    rows = [parse_result(path, results_root) for path in thesan_json_paths(results_root, include_canonical)]
    ...
    for key in sorted(stem_groups):
        ...
        for run_index, row in enumerate(group, start=1):
            ...
            row["canonical_path"] = canonical_path(results_root, row, run_index)
            ...
            move_rows.append(
                {
                    ...
                    "action": "copy",
                    "reason": "canonical-thesan-layout",
                }
            )
            if len(group) > 1:
    ...
    for plot_path in thesan_plot_paths(results_root):
        ...
        move_rows.append(
            {
                ...
                "action": "copy",
                "reason": "canonical-thesan-analysis-layout",
            }
        )
...
def main() -> None:
    ...
    parser.add_argument("--include-canonical", action="store_true", help="Also audit existing results/thesan JSONs.")
    ...
    write_csv(args.manifest_dir / "thesan_results_manifest.csv", MANIFEST_COLUMNS, public_manifest_rows)
    write_csv(args.manifest_dir / "thesan_duplicate_report.csv", DUPLICATE_COLUMNS, duplicate_rows)
    write_csv(args.manifest_dir / "thesan_move_plan.csv", MOVE_COLUMNS, move_rows)
    if args.apply:
        apply_moves(move_rows, args.move)
    print(
        f"Audited {len(manifest_rows)} Thesan JSON files; "
        f"wrote {len(move_rows)} move-plan rows and {len(duplicate_rows)} duplicate rows."
    )
