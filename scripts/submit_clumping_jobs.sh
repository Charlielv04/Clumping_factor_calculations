...
for backend in ${BACKENDS}; do
  if [[ "${backend}" == "voronoi-transmission" ]]; then
    if [[ " ${PARTICLES} " != *" gas "* || " ${PARTICLES} " == *" dm "* ]]; then
      echo "voronoi-transmission is gas-only; set PARTICLES=gas." >&2
      exit 1
    fi
    if [[ -z "${SIGMA_BAR_ION_CM2}" || -z "${SIGMA_BAR_ION_SOURCE}" ]]; then
      echo "BACKENDS includes voronoi-transmission; set SIGMA_BAR_ION_CM2 and SIGMA_BAR_ION_SOURCE." >&2
      exit 1
    fi
  fi
done
