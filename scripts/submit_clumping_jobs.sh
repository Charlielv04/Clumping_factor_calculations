...
for backend in ${BACKENDS}; do
  if [[ "${backend}" == "raw-transmission" || "${backend}" == "voronoi-transmission" ]]; then
    if [[ "${SIGMA_BAR_ION_MODE}" != "explicit" && "${SIGMA_BAR_ION_MODE}" != "thesan-photon-groups" ]]; then
      echo "SIGMA_BAR_ION_MODE must be explicit or thesan-photon-groups." >&2
      exit 1
    fi
    if [[ " ${PARTICLES} " != *" gas "* || " ${PARTICLES} " == *" dm "* ]]; then
      echo "transmission backends are gas-only; set PARTICLES=gas." >&2
      exit 1
    fi
    if [[ "${SIGMA_BAR_ION_MODE}" == "explicit" && -z "${SIGMA_BAR_ION_CM2}" ]]; then
      echo "Transmission backends in explicit mode require SIGMA_BAR_ION_CM2." >&2
      exit 1
    fi
    if [[ -z "${SIGMA_BAR_ION_SOURCE}" ]]; then
      echo "Transmission backends require SIGMA_BAR_ION_SOURCE." >&2
      exit 1
    fi
  fi
done
...
submit_one() {
  ...
  qsub_args=(
    ...
    -l "walltime=${walltime}" \
      -v "PROJECT_DIR=${PROJECT_DIR},BASE_PATH=${BASE_PATH},SIMULATION_NAME=${SIMULATION_NAME},RESULTS_LAYOUT=${RESULTS_LAYOUT},RESULTS_FAMILY=${RESULTS_FAMILY},SOURCE_CAMPAIGN=${SOURCE_CAMPAIGN},CONDA_ENV=${CONDA_ENV},SNAPSHOT=${SNAPSHOT},RADIUS_BINS=${RADIUS_BINS},RADIUS_BIN_BATCH_SIZE=${RADIUS_BIN_BATCH_SIZE},RADIUS_MODE=${RADIUS_MODE},MAS=${MAS},THREADS=${THREADS},NCPUS=${NCPUS},RESOURCE_SIZE=${resource_size},MEMORY_LIMIT=${mem},MEMORY_SAFETY_FRACTION=${MEMORY_SAFETY_FRACTION},SUMMARY_CACHE=${SUMMARY_CACHE},SUMMARY_CACHE_DIR=${SUMMARY_CACHE_DIR},WORK_PARTITION=${WORK_PARTITION},MAX_FILE_READERS=${MAX_FILE_READERS},CLUMPING_MAX_GRID_CELLS=${CLUMPING_MAX_GRID_CELLS},RUN_LABEL=${run_label},LOAD_MODE=${LOAD_MODE},CHUNK_SIZE=${CHUNK_SIZE},MAX_FULL_LOAD_GB=${MAX_FULL_LOAD_GB},PROGRESS_INTERVAL=${PROGRESS_INTERVAL},VERBOSE=${VERBOSE},SIGMA_BAR_ION_MODE=${SIGMA_BAR_ION_MODE},PARTICLE=${particle},BACKEND=${backend},GRID=${grid},TARGET_PARTICLE_TYPE=${TARGET_PARTICLE_TYPE},TARGET_BACKEND=${TARGET_BACKEND},MASK_PARTICLE_TYPE=${MASK_PARTICLE_TYPE},MASK_BACKEND=${MASK_BACKEND},TARGET_RADIUS_MODE=${TARGET_RADIUS_MODE},MASK_RADIUS_MODE=${MASK_RADIUS_MODE},SIGMA_BAR_ION_CM2=${SIGMA_BAR_ION_CM2},SIGMA_BAR_ION_SOURCE=${SIGMA_BAR_ION_SOURCE},VORONOI_NEIGHBORS=${VORONOI_NEIGHBORS},VORONOI_GRADIENT_BATCH_SIZE=${VORONOI_GRADIENT_BATCH_SIZE},RAW_CLUMPING_MODE=${RAW_CLUMPING_MODE},RAW_HII_SOURCE=${RAW_HII_SOURCE},RAW_ELECTRON_SOURCE=${RAW_ELECTRON_SOURCE},RAW_CONSTANT_ELECTRON_ABUNDANCE=${RAW_CONSTANT_ELECTRON_ABUNDANCE},RAW_HYDROGEN_MASS_FRACTION=${RAW_HYDROGEN_MASS_FRACTION},THRESHOLD_MIN=${THRESHOLD_MIN},THRESHOLD_MAX=${THRESHOLD_MAX},THRESHOLD_COUNT=${THRESHOLD_COUNT}" \
