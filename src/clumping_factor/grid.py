...
def _assignment_indices_and_weights(coords: np.ndarray, lbox: float, grid_size: int, mas: str) -> Iterable[tuple[np.ndarray, np.ndarray]]:
    scaled = np.mod(np.asarray(coords, dtype=np.float64), float(lbox)) * (int(grid_size) / float(lbox))
    if mas == "CIC":
        base = np.floor(scaled).astype(np.int64)
        fraction = scaled - base
        axis_indices = [
            (base[:, axis] % grid_size, (base[:, axis] + 1) % grid_size)
            for axis in range(3)
        ]
        axis_weights = [(1.0 - fraction[:, axis], fraction[:, axis]) for axis in range(3)]
        offsets = range(2)
    elif mas == "TSC":
    ...
    for ox in offsets:
        for oy in offsets:
            for oz in offsets:
                flat_indices = (
                    axis_indices[0][ox] * grid_size**2
                    + axis_indices[1][oy] * grid_size
                    + axis_indices[2][oz]
                )
...
def _add_deposited_mass(
    ...
    for flat_indices, assignment_weights in _assignment_indices_and_weights(coords, lbox, grid_size, mas):
