...
In the context of reionization, the clumping factor measures how inhomogeneous matter is relative to a uniform distribution. Since recombination rates scale approximately with density squared, small-scale density structure can strongly affect the ionization state of the intergalactic medium (IGM). A reliable clumping factor calculation therefore needs both physical care, because gas and dark matter trace structure differently, and computational care, because modern simulations are large enough that naive full-snapshot processing can become impractical.

The work described here had two connected goals. The first was scientific: to build the background needed to understand what is being measured, why different dark matter models may change the relevant density structure, how cosmological simulations store particle and gas data, and how clumping factors enter reionization calculations. The second was practical: to develop the initial implementation into a reproducible analysis workflow with command-line tools, tests, cluster submission scripts, benchmark outputs, and documentation.

\section{Scientific Background}

The first two weeks and part of the third week were primarily devoted to background research. This stage covered the physical role of dark matter in structure formation, how cosmological simulations represent matter fields, the basic history and physics of cosmic reionization, and the reason clumping factors appear in recombination-rate estimates. A central lesson from this phase was that a clumping factor is not just a software diagnostic: it depends on the matter component being measured, the density field used to define the IGM, the smoothing or gridding method, and the averaging convention.

Modern cosmological simulations evolve matter from early-Universe initial conditions into the nonlinear structures observed at later times. They are especially useful because they connect assumptions about cosmology and dark matter to the distribution of galaxies, halos, gas, and the IGM \cite{vogelsberger2019}. In this project, the relevant point is that changing the dark matter model can modify small-scale structure formation. Those changes can alter the abundance of small halos, the density field around collapsed structures, and the distribution of gas that participates in recombination and absorption.
...
\end{equation}
If the density field is uniform, \(C=1\). If the field contains dense structures, \(C>1\). For reionization, this is important because dense regions recombine more efficiently and therefore increase the number of ionizing photons required to keep the Universe ionized. Observational work has also connected the clumping factor to quantities such as the ionizing mean free path and photoionization rate near the end of reionization \cite{davies2024}. This connection is important for the long-term goal of comparing simulation predictions with observational constraints.

However, the clumping factor is not uniquely defined. It depends on the averaging volume, the gas included in the IGM, and whether the calculation uses a hard overdensity threshold or a more physically motivated selection. Recent work on recombination clumping factors shows that changing the IGM definition can significantly change the inferred value of \(C\) \cite{oku2025}. This motivates treating the definition of the clumping factor as part of the scientific model, not only as an implementation detail.

The Lyman-alpha forest is also relevant to this project because it gives observational access to the neutral hydrogen distribution through absorption along lines of sight to distant sources. Instead of directly observing the full three-dimensional density field, one observes transmitted flux and absorption statistics. We plan to use Lyman-alpha forest calculations as a bridge between simulation fields and observations: the goal is to determine whether an observationally inferred clumping factor can be defined from Lyman-alpha quantities and how that quantity relates to the more standard density-based clumping factor computed directly from the simulations. Modeling the Lyman-alpha forest requires care because the absorption depends not only on density, but also on gas temperature, ionization state, velocities, and the relation between baryons and the underlying matter distribution \cite{sinigaglia2021}.
...
The validation work was designed to make the calculation reliable enough for comparisons between dark matter models. Instead of only checking that a script ran to completion, the tests verify that command-line options are interpreted consistently, grids are constructed as expected, chunked loading agrees with the intended data flow, native gas-cell calculations preserve the correct volume-weighted averaging convention, and plotting operates on the saved result files. This gives confidence that later differences between simulations are more likely to reflect physics than accidental differences in how a run was configured.
...
Figure~\ref{fig:thesan-clumping} shows an example clumping-consistency diagnostic. The goal of this type of figure is to verify that the calculated clumping factors remain comparable when the same physical calculation is performed under different numerical choices.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{results/analysis/thesan-1-vs-2-scaling/clumping_consistency.png}
    \caption{Example clumping-consistency diagnostic. The figure is used to check whether different numerical choices preserve the same physical clumping-factor behavior.}
    \label{fig:thesan-clumping}
\end{figure}

Additional analysis focused on the stability of the dark-matter calculation across grid sizes and parallel configurations. This work is useful for separating physical differences from numerical and computational effects. Figure~\ref{fig:dm-grid-scaling} shows a grid-scaling diagnostic from these runs.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\linewidth]{results/analysis/dm-pylians-grid-thread-scaling/grid_scaling.png}
    \caption{Dark-matter grid-scaling diagnostic. This type of comparison helps identify how resolution and parallel configuration affect both performance and numerical behavior.}
    \label{fig:dm-grid-scaling}
\end{figure}

\section{Conclusion}

The first month of the internship produced both scientific understanding and a practical computational workflow. The early research phase established why clumping factors matter for reionization and why the definition of the density field, smoothing method, and IGM selection all affect the result. The initial scripts then provided the first working implementation. The later development transformed that prototype into a tested package with command-line tools, chunked and parallel execution, benchmark diagnostics, result analysis, and documentation.
