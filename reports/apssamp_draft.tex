...
The first approximate estimator presented by Davies et al. assumes ionization equilibrium. This corresponds to the assumption
$$
...
The results are shown in Figure~\ref{fig:ionization-equilibrium}. With a sufficiently strict ionization cutoff, the equilibrium relation is satisfied for each of the three overdensity cutoffs considered.
...
In calculating the first estimator, Davies et al. additionally approximate the mean free path as
$$
\lambda_{\mathrm{mfp}}=
...
Davies et al. introduce a second estimator, which also relies on the ionization-equilibrium assumption studied above. In addition, they define an effective photon interaction rate,
$$
\Gamma_{\gamma}=\frac{c}{\lambda_{\mathrm{mfp}}},
$$
such that the photoionization rate per unit volume balances the recombination rate. Ionization equilibrium then gives the additional assumption
$$
...
This section addresses the main objective of this work: whether the clumping factor can serve as an observable for constraining the parameter spaces of different dark-matter models. We compare the predictions of the AIDA-TNG simulations for cold dark matter (CDM), warm dark matter (WDM), self-interacting dark matter (SIDM), and velocity-dependent self-interacting dark matter (vSIDM).
...
We first compare the gas clumping factors at redshift zero, using snapshot 099 and the native Voronoi-cell, volume-weighted calculation. Figures~\ref{fig:aida-z0-gas-clumping-absolute} and~\ref{fig:aida-z0-gas-clumping-relative} show the absolute clumping factors and the corresponding relative difference from the CDM result,
$$
\frac{C_{\rm CDM}-C_{\rm model}}{C_{\rm CDM}}.
$$

The corresponding dark-matter measurements are shown in Figures~\ref{fig:aida-z0-dm-clumping-absolute} and~\ref{fig:aida-z0-dm-clumping-relative}; because dark matter is particle based, these use the standardized $512^3$ CIC, top-hat--smoothed grid calculation.
...
To examine the redshift dependence, we follow the clumping factor at the nominal threshold $\Delta_{\rm max}=20$ and the CDM-relative difference integrated over the full overdensity interval $0\leq\Delta\leq25$. Colours distinguish dark-matter models; dashed and solid lines denote L35n1080 and L75n910, respectively. Each curve uses the snapshots available for that model, without interpolation where an output is unavailable. The gas result is calculated directly from the native Voronoi cells, whereas the dark-matter result uses the standardized $512^3$ CIC, top-hat--smoothed grid calculation.
...
\begin{figure}[t]
    ...
    \caption{Redshift evolution of the overdensity-integrated CDM-relative gas-clumping difference, $\Delta_C=\int_0^{25}(C_{\rm CDM}-C_{\rm model})/C_{\rm CDM}\,d\Delta$. Dashed lines show L35n1080 and solid lines L75n910.}
...
\begin{figure}[t]
    ...
    \caption{Redshift evolution of the overdensity-integrated CDM-relative dark-matter clumping difference, $\Delta_C=\int_0^{25}(C_{\rm CDM}-C_{\rm model})/C_{\rm CDM}\,d\Delta$. Dashed lines show L35n1080 and solid lines L75n910.}
...
We first repeat the grid-size comparison using the gas fields of the AIDA-TNG simulations. Once again, the gas cells are treated as point-like elements carrying their cell masses and then deposited into the grid. We use cloud-in-cell as the mass-assignment scheme and apply spherical Pylians smoothing to the resulting density field. To compare the gridded estimates with the direct Voronoi-cell result over the full overdensity range, we use the normalized gridding-deviation factor introduced in Section~\ref{sec:density-grid-tests},
\begin{equation}
...
For a controlled test, we processed the dark-matter snapshot at $z=5.49$ of
Thesan-2 on a $512^3$ grid three times with one worker and three times with
16 workers.  All calculations used the same input, settings, and an inactive
summary cache.  The median end-to-end time decreased from 13.90 to
2.56 minutes, corresponding to a speedup of $5.44$ (Fig.~\ref{fig:parallel-timing}).
The requested and effective worker counts were equal in every run.

\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{figures/parallelization_benchmark.png}
\caption{Controlled wall-time measurement for the Thesan-2 dark-matter
snapshot at $z=5.49$ on a $512^3$ grid.  Points give the median of three
cache-free repetitions and error bars show the full repeat range.}
\label{fig:parallel-timing}
\end{figure*}

The serial and parallel calculations produced identical clumping-factor
curves at the stored precision.  The selected-cell totals differed by fewer
than eight cells out of $512^3$, which is negligible for the measurements in
this work.  Parallel execution therefore acts only as a computational
